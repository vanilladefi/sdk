 ...
 */
export const weth = getAllTokens(VanillaVersion.V1_0)?.find((token) => token.chainId === String(chainId) && token.symbol === defaultWeth.symbol) || defaultWeth;
/**
 * Equals USDC from getAllTokens if found, defaultUsdc if not
 */
export const usdc = getAllTokens(VanillaVersion.V1_0)?.find((token) => token.chainId === String(chainId) && token.symbol === 'USDC') || defaultUsdc;
/**
 ...
 */
export function getAllTokens(version) {
    // Convert TokenList format to our own format
    const defaultTokens = uniswapTokens?.tokens
    ...
    const vanillaTokens = additionalTokens
        .map((t) => JSON.parse(JSON.stringify(t))) // Needed for casting to Token[] format
        .map((t) => ({
        ...t,
        chainId: String(t.chainId),
    ...
    return allTokens
        .filter((token) => token.chainId === String(chainId) &&
 ...
 */
export function padUniswapTokenToToken(input) {
    const token = {
        pairId: null,
        logoColor: null,
        ...input,
 ...
 */
export function convertVanillaTokenToUniswapToken(input) {
    const token = new UniswapToken(Number(input.chainId), input.address, Number(input.decimals), input.symbol, input.name);
    return token;
}
