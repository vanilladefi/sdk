import { CurrencyAmount, Fraction, Price, Token as UniswapToken, TokenAmount, TradeType, } from '@uniswap/sdk-core';
...
import { chainId, conservativeGasLimit, tradeContractAddresses, ethersOverrides, getFeeTier, } from '../../contracts';
 ...
 */
export const UniswapOracle = (oracle) => ({
    swap(tokenIn, tokenOut) {
        return {
            swapParamsIn(amountIn, fee) {
                return {
                    tokenIn,
                    tokenOut,
                    fee: fee,
            ...
            async estimateAmountOut(amountIn, fee) {
                let amountOut = BigNumber.from(0);
                try {
                    const swapParams = this.swapParamsIn(amountIn, fee);
                    amountOut = await oracle.callStatic.quoteExactInputSingle(swapParams.tokenIn, swapParams.tokenOut, fee, swapParams.amountIn, swapParams.sqrtPriceLimitX96, {
            ...
            async estimateAmountIn(amountOut, fee) {
                let amountIn = BigNumber.from(0);
                try {
                    const swapParams = this.swapParamsOut(amountOut, fee);
                    amountIn = await oracle.callStatic.quoteExactOutputSingle(swapParams.tokenIn, swapParams.tokenOut, fee, swapParams.amountOut, swapParams.sqrtPriceLimitX96, {
                        gasLimit: conservativeGasLimit,
                    });
                }
                catch (e) {
 ...
 */
export const buy = async ({ amountPaid, amountReceived, tokenReceived, signer, blockDeadline, feeTier, gasLimit, }) => {
    const vnl1_1Addr = isAddress(tradeContractAddresses.vanilla[VanillaVersion.V1_1]?.router || '');
    if (vnl1_1Addr && tokenReceived?.address && signer && feeTier) {
        ...
        const receipt = await router.executePayable([router.interface.encodeFunctionData('buy', [orderData])], { value: amountPaid, gasLimit: usedGasLimit });
        return receipt;
    }
    else {
 ...
 * @returns an ethersjs Transaction object
 */
export const sell = async ({ amountPaid, amountReceived, tokenPaid, signer, blockDeadline, feeTier, gasLimit, }) => {
    const vnl1_1Addr = isAddress(tradeContractAddresses.vanilla[VanillaVersion.V1_1]?.router || '');
    if (vnl1_1Addr && tokenPaid?.address && signer && feeTier) {
        ...
        const receipt = await router.executePayable([router.interface.encodeFunctionData('sell', [orderData])], { gasLimit: usedGasLimit });
        return receipt;
    }
    else {
        return Promise.reject('No Vanilla v1.1 router on used chain!');
...
class V3Trade {
    constructor(inputAmount, outputAmount, tradeType) {
        ...
        this.price = new Price(inputAmount.token, outputAmount.token, inputAmount.raw.toString(), outputAmount.raw.toString());
        this.route = null;
    }
    worstExecutionPrice() {
        return this.price;
 ...
 */
export async function constructTrade(signerOrProvider, amountToTrade, // Not amountPaid because of tradeType
tokenReceived, tokenPaid, tradeType) {
    const defaultFeeTier = FeeAmount.MEDIUM;
    try {
        const tokenToTrade = tradeType === TradeType.EXACT_OUTPUT ? tokenReceived : tokenPaid;
        const quotedToken = tradeType === TradeType.EXACT_OUTPUT ? tokenPaid : tokenReceived;
        ...
        const feeTier = getFeeTier(tokenReceived.fee) ||
            getFeeTier(tokenPaid.fee) ||
            defaultFeeTier;
        const uniV3Oracle = Quoter__factory.connect(tradeContractAddresses.uniswap.v3.quoter || '', signerOrProvider);
        const swapOperation = UniswapOracle(uniV3Oracle).swap(isAddress(tokenPaid.address) || tokenPaid.address, isAddress(tokenReceived.address) || tokenReceived.address);
        let quote = BigNumber.from(0);
        if (tradeType === TradeType.EXACT_INPUT) {
            quote = await swapOperation.estimateAmountOut(parsedAmountTraded, feeTier.valueOf());
        }
        else {
            quote = await swapOperation.estimateAmountIn(parsedAmountTraded, feeTier.valueOf());
        }
        ...
        if (parsedQuote && parsedAmountTraded) {
            return new V3Trade(tradeType === TradeType.EXACT_INPUT ? parsedAmountTraded : parsedQuote, tradeType === TradeType.EXACT_INPUT ? parsedQuote : parsedAmountTraded, tradeType);
        }
        else {
            return null;
        }
...
export function tryParseAmount(value, currency) {
    ...
    try {
        const convertedToken = new UniswapToken(chainId, getAddress(currency.address), Number(currency.decimals));
