import { VanillaV1MigrationState__factory } from '@vanilladefi/trade-contracts/typechain/vanilla_v1.1/factories/VanillaV1MigrationState__factory';
import { BigNumber, constants, utils } from 'ethers';
import { keccak256 } from 'ethers/lib/utils';
import { MerkleTree } from 'merkletreejs';
const toSnapshotState = (state, event) => {
...
export const toKeccak256Leaf = (balance) => utils.solidityKeccak256(['address', 'string', 'uint256'], [balance.address, ':', balance.amount]);
export const snapshot = async (token01, token02) => {
    const snapshotBlock = await token02
        .migrationState()
        .then((address) => VanillaV1MigrationState__factory.connect(address, token02.provider).blockNumber());
    const tokenTransfers = snapshotBlock.eq(0)
        ? await token01.queryFilter(token01.filters.Transfer(null, null, null))
        : await token01.queryFilter(token01.filters.Transfer(null, null, null), 0, snapshotBlock.toNumber());
    const byBlockIndexOrder = (a, b) => a.blockNumber - b.blockNumber || a.logIndex - b.logIndex;
    ...
    const blockAtSnapshot = await token01.provider.getBlock(snapshotState.blockNumber);
    snapshotState.timeStamp = blockAtSnapshot.timestamp;
    const leaves = Object.entries(snapshotState.accounts).map(([address, amount]) => ({
        address,
        amount,
        hash: toKeccak256Leaf({ address, amount }),
    }));
    const merkleTree = new MerkleTree(leaves.map((x) => x.hash), keccak256, { sortPairs: true, hashLeaves: false });
